...
ENTRY_TIME = "15:45"  # Near market close
EXIT_TIME = "09:31"   # Post market open
BUY_THRESHOLD = 30    # RSI buy threshold
SELL_THRESHOLD = 70   # RSI sell threshold
MACD_FAST = 12        # MACD fast EMA period
MACD_SLOW = 26        # MACD slow EMA period
MACD_SIGNAL = 9       # MACD signal line period
BB_PERIOD = 20        # Bollinger Bands period
BB_STD = 2            # Bollinger Bands standard deviation
DELTA_TARGET = 0.4    # Target delta for call options
MIN_VOLUME = 1000000  # Minimum stock volume for liquidity
...
def fetch_options(ticker):
    try:
        ...
        if not expirations:
            return None
        expiration = expirations[0]  # Closest expiration
        opts = stock.option_chain(expiration)
        calls = opts.calls
        calls['Delta'] = calls['lastPrice'] / calls['strike']  # Simplified delta
        return calls
    except Exception as e:
...
def evaluate_trade(df, ticker):
    ...
    if volume < MIN_VOLUME:
        return None  # Skip low-liquidity tickers

    # Buy signal: RSI < threshold, price > EMA20, MACD bullish, near BB lower
    if (rsi < BUY_THRESHOLD and price > ema20 and
        ...
        signal_score = BUY_THRESHOLD - rsi  # Higher score for lower RSI
...
def fetch_market_trends():
    ...
    for etf in sector_etfs:
        ...
        trends[etf] = round(df['Close'].iloc[-1], 2) if not df.empty else None
    trends['VIX'] = round(vix['Close'].iloc[-1], 2) if not vix.empty else None
    return trends
...
if not df_results.empty:
    # Show top 10 BUY signals, sorted by Signal_Score
